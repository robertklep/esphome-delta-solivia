...
protected:
  void parse_frame_(const uint8_t* frame, std::size_t pos) override {
    ...
    this->publish_sensor_("Software_rev_sc_major",
            frame[pos++]);  // Major SC revision
    this->publish_sensor_("Software_rev_sc_minor",
    ...
    for (int input = 1; input <= 3; ++input) {
      ...
      this->publish_sensor_(CONF_INV_SOLAR_VOLTAGE_PREFIX + std::to_string(input), this->extract_int16(&frame[pos])); // Volts
      pos += 2;
      // Solar current at input 1 A 2[UINT16] 1 = 0,1A
      this->publish_sensor_(CONF_INV_SOLAR_CURRENT_PREFIX + std::to_string(input), this->extract_int16(&frame[pos]) * 0.1); // Amps
      pos += 2;
    }

    for (int phase = 1; phase <= 3; ++phase) {
      // AC current - L1 A 2[UINT16] 1 = 0,1A
      this->publish_sensor_(CONF_INV_AC_CURRENT_PHASE_PREFIX + std::to_string(phase), this->extract_int16(&frame[pos]) / 10.0); // A
      pos += 2;
      // AC voltage - L1 V 2[UINT16] 1= 1V
      this->publish_sensor_(CONF_INV_AC_VOLTAGE_PHASE_PREFIX + std::to_string(phase), this->extract_int16(&frame[pos]) ); // V
      pos += 2;
      ...
      this->publish_sensor_("ac_reactive_power_phase_" + std::to_string(phase), this->extract_int16(&frame[pos])); // W
      pos += 2;
    }
    ...
    this->publish_sensor_("ambient_temp", this->extract_int16(&frame[pos])); // C
    pos += 2;
    // Temperature heatsink °C 2[INT16] 1 = 1°C
    this->publish_sensor_("heatsink_temp", this->extract_int16(&frame[pos])); // C
    pos += 2;
    // Supplied ac energy total Wh 8[UINT64] 1 = 1Wh
    this->publish_sensor_(CONF_INV_SUPPLIED_AC_ENERGY_TOTAL, this->extract_int64(&frame[pos])); // kWh
    pos += 8;
    // Inverter runtime total Minutes 4[UINT32] 1 = 1 minute
    this->publish_sensor_(CONF_INV_RUNTIME_TOTAL, this->extract_int32(&frame[pos]));
    pos += 4;

    // Status 1 4[UINT32] bit description
    status_1 = this->extract_int32(&frame[pos]);
    pos += 4;
    // Status 2 4[UINT32] bit description
    status_2 = this->extract_int32(&frame[pos]);
    pos += 4;
    // Status 3 4[UINT32] bit description
    status_3 = this->extract_int32(&frame[pos]);
    pos += 4;
    // Status 4 4[UINT32] bit description
    status_4 = this->extract_int32(&frame[pos]);
    pos += 4;

    this->parse_status(status_1, status_2, status_3, status_4);

    // Internal status 1 4[UIT32] reserved bit description
    this->publish_sensor_("internal_status_1", this->extract_int32(&frame[pos])); 
    pos += 4;
    // Internal status 2 4[UINT32] reserved bit description
    this->publish_sensor_("internal_status_2", this->extract_int32(&frame[pos])); 
    pos += 4;
    // Internal status 3 4[UINT32] reserved bit description
    this->publish_sensor_("internal_status_3", this->extract_int32(&frame[pos])); 
    pos += 4;
    // Internal status 4 4[UINT32] reserved bit description
    this->publish_sensor_("internal_status_4", this->extract_int32(&frame[pos])); 
    pos += 4;
    // Supplied ac energy day Wh 8[UINT64] 1 = 1Wh
    this->publish_sensor_(CONF_INV_SUPPLIED_AC_ENERGY_TODAY, this->extract_int64(&frame[pos]));  // Wh    
    pos += 8;
    // Inverter runtime day Minutes 4[UINT32] 1 = 1 minute
    this->publish_sensor_(CONF_INV_RUNTIME_TODAY, this->extract_int32(&frame[pos])*60);
    pos += 4;
    // reserved 67
  }
  ...
  void parse_status(uint32_t status_1, uint32_t status_2, uint32_t status_3, uint32_t status_4){
    ...
    this->publish_text_sensor_(CONF_INV_STATUS_TEXT, status_text);
  }
public:
